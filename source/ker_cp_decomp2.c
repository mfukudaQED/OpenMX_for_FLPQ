  ...
  if (0){
	...
	for (k=0; k<Nd1; k++){
	  ...
	  for (i1=0; i1<Ng1; i1++){
	    ...
	    else if (i==1) { fx1 = exp(-0.6*x);          fx2 = exp(-0.6*x);            }
	    else if (i==2) { fx1 = exp(-2.0*x);          fx2 = exp(-3.0*x);            }
	    else if (i==3) { fx1 = exp(-8.0*x);          fx2 = exp(-8.0*x);            }
	    ...
	    for (j1=0; j1<Ng1; j1++){
	      ...
	      else if (j==1) { fy1 = exp(-0.6*y);  fy2 = exp(-0.6*y);   }
	      else if (j==2) { fy1 = exp(-2.0*y);  fy2 = exp(-3.0*y);   }
	      else if (j==3) { fy1 = exp(-8.0*y);  fy2 = exp(-8.0*y);   }
	      
	      for (k1=0; k1<Ng1; k1++){
	      
		z = xmin + (double)k1*dx1;

		if      (k==0) { fz1 = exp(-0.1*z); fz2 = exp(-0.1*z);  }
		else if (k==1) { fz1 = exp(-0.6*z); fz2 = exp(-0.6*z);  }
		else if (k==2) { fz1 = exp(-2.0*z); fz2 = exp(-3.0*z);  }
		else if (k==3) { fz1 = exp(-8.0*z); fz2 = exp(-8.0*z);  }
	...
	if      (I==0) { fx1 = exp(-0.01*x*x);   fx2 = exp(-0.01*x*x);   }
	else if (I==1) { fx1 = exp(-0.2*x*x);   fx2 = exp(-0.2*x*x);   }
	else if (I==2) { fx1 = exp(-2.0*x*x);   fx2 = exp(-2.0*x*x);   }
	else if (I==3) { fx1 = exp(-8.0*x*x);   fx2 = exp(-8.0*x*x);   }
	else if (I==4) { fx1 = exp(-12.0*x*x);   fx2 = exp(-12.0*x*x);   }

	for (j1=0; j1<Ng1; j1++){
	    
	  y = xmin + (double)j1*dx1;

	  if      (I==0) { fy1 = exp(-0.01*y*y);  fy2 = exp(-0.01*y*y);   }
	  else if (I==1) { fy1 = exp(-0.2*y*y);  fy2 = exp(-0.2*y*y);   }
	  else if (I==2) { fy1 = exp(-2.0*y*y);  fy2 = exp(-2.0*y*y);   }
	  else if (I==3) { fy1 = exp(-8.0*y*y);  fy2 = exp(-8.0*y*y);   }
	  else if (I==4) { fy1 = exp(-12.0*y*y);  fy2 = exp(-12.0*y*y);   }
	      
	  for (k1=0; k1<Ng1; k1++){
	      
	    z = xmin + (double)k1*dx1;

	    if      (I==0) { fz1 = exp(-0.01*z*z); fz2 = exp(-0.01*z*z);  }
	    else if (I==1) { fz1 = exp(-0.2*z*z); fz2 = exp(-0.2*z*z);  }
	    else if (I==2) { fz1 = exp(-2.0*z*z); fz2 = exp(-2.0*z*z);  }
	    else if (I==3) { fz1 = exp(-8.0*z*z); fz2 = exp(-8.0*z*z);  }
	    else if (I==4) { fz1 = exp(-12.0*z*z); fz2 = exp(-12.0*z*z);  }
