...
//static int** natn_C;
//static int* FNAN_C;
//static void set_FNAN_in_cell(int* FNAN_C, int** natn_C);
//static void free_FNAN_in_cell(int* FNAN_C, int** natn_C);
                                ...
                                double ****CntOLP)
{
  ...
  int GA_AN,Anum;
  int ii,ij,ik,Rn,AN;
  int num0,num1,mul,m,wan1,TNO1,Gc_AN;
  int Gh_AN, wan2, TNO2;
  ...
  ****************************************************/
  ...
  //set_FNAN_in_cell(FNAN_C, natn_C);
...
#pragma omp parallel
  ...
  //for (i=0; i<atomnum; i++) tmpelem[i]=(dcomplex***)malloc(sizeof(dcomplex**)*(FNAN[i+1]+1));
  //for (i=0; i<atomnum; i++) for (j=0; j<=FNAN[i+1]; j++) {
  //  tmpelem[i][j]=(dcomplex**)malloc(sizeof(dcomplex*)*Norbperatom[i]);
  //  for (k=0; k<Norbperatom[i]; k++) {
  //    tmpelem[i][j][k]=(dcomplex*)malloc(sizeof(dcomplex)*Norbperatom[natn[i+1][j]-1]);
  //  }
  //}

  //tmpelem2=(dcomplex****)malloc(sizeof(dcomplex***)*atomnum);
  //for (i=0; i<atomnum; i++) tmpelem2[i]=(dcomplex***)malloc(sizeof(dcomplex**)*atomnum);
  //for (i=0; i<atomnum; i++) for (j=0; j<atomnum; j++) {
  //  tmpelem2[i][j]=(dcomplex**)malloc(sizeof(dcomplex*)*Norbperatom[i]);
  //  for (k=0; k<Norbperatom[i]; k++) {
  //    tmpelem2[i][j][k]=(dcomplex*)malloc(sizeof(dcomplex)*Norbperatom[j]);
  //  }
  //}
...
#pragma omp parallel for \
  shared( count_k4S, atomnum, order_GA, WhatSpecies, Spe_Total_CNO, FNAN,\
         natn, ncn, mapN2rn, num_mapn2N, mapn2N, rlist, a, b, c, K,\
         tmp_weight, unfold_mapN2n, tabr4RN, kj_v, S1, atv_ijk, _MapR, myid)\
  private( index_thread, k, MA, GA_AN, wan1, TNO1, h_AN, Gh_AN, Rn, wan2, TNO2,\
          ir, r, ii, l, MB, mb, r0x, r0y, r0z, r0, phase1, phase2, MO, NO, dtmp)
          for (MA=0; MA<atomnum; MA++) {
            ...
            for (h_AN=0; h_AN<=FNAN[GA_AN]; h_AN++) {
              ...
              ir = mapN2rn[MapR(atv_ijk[Rn][1],atv_ijk[Rn][2],atv_ijk[Rn][3])][Gh_AN-1];
          ...
          if (spin==0) {
            fprintf(fp_EV,"%f %f %10.7f\n",kdis,kj_e*eV2Hartree,fabs(sumallorb)/coe);
          }
          else {
          ...
          if (spin==0){
            fprintf(fp_EV1,"%f %f ", kdis,kj_e*eV2Hartree);
          }
          else {
            fprintf(fp_EV3,"%f %f ",kdis,kj_e*eV2Hartree);
          }
...
//  for (i=0; i<NR; i++) for (j=0; j<atomnum; j++) for (k=0; k<=FNAN[j+1]; k++) for (l=0; l<Norbperatom[j]; l++) free(Elem[i][j][k][l]);
//  for (i=0; i<NR; i++) for (j=0; j<atomnum; j++) for (k=0; k<=FNAN[j+1]; k++) free(Elem[i][j][k]);
//  for (i=0; i<NR; i++) for (j=0; j<atomnum; j++) free(Elem[i][j]);
//  for (i=0; i<NR; i++) free(Elem[i]); free(Elem);
//  for (i=0; i<atomnum; i++) for (j=0; j<=FNAN[i+1]; j++) for (k=0; k<Norbperatom[i]; k++)
//    free(tmpelem[i][j][k]);
//  for (i=0; i<atomnum; i++) for (j=0; j<=FNAN[i+1]; j++) free(tmpelem[i][j]);
//  for (i=0; i<atomnum; i++) free(tmpelem[i]); free(tmpelem);
//
//  for (i=0; i<atomnum; i++) for (j=0; j<atomnum; j++) for (k=0; k<Norbperatom[i]; k++)
//    free(tmpelem2[i][j][k]);
//  for (i=0; i<atomnum; i++) for (j=0; j<atomnum; j++) free(tmpelem2[i][j]);
//  for (i=0; i<atomnum; i++) free(tmpelem2[i]); free(tmpelem2);
  ...
  //free_FNAN_in_cell(FNAN_C, natn_C);
...
#pragma omp parallel for \
  shared( count_k4S, atomnum, order_GA, WhatSpecies, Spe_Total_CNO, FNAN,\
         natn, ncn, mapN2rn, num_mapn2N, mapn2N, rlist, a, b, c, K,\
         tmp_weight, unfold_mapN2n, tabr4RN, kj_v, kj_v1, S1, atv_ijk, _MapR, myid)\
  private( index_thread, k, MA, GA_AN, wan1, TNO1, h_AN, Gh_AN, Rn, wan2, TNO2,\
          ir, r, ii, l, MB, mb, r0x, r0y, r0z, r0, phase1, phase2, MO, NO, dtmp, dtmp1)
          for (MA=0; MA<atomnum; MA++) {
            index_thread = omp_get_thread_num();
            k = count_k4S[MA];
            GA_AN = order_GA[MA+1];
...
void buildrnmap(const int* mapN2n) {
  ...
  mapN2rn=(int**)malloc(sizeof(int*)*NR);
  for (iR=0; iR<NR; iR++){
    mapN2rn[iR]=(int*)malloc(sizeof(int)*atomnum);
  }
  ...
  //if(myid == Host_ID){
  //  printf("GA_AN, iRn, ga_an, irn\n");fflush(stdout);
  //  for (iR=0; iR<NR; iR++){
  //    for (GA_AN=1; GA_AN<=atomnum; GA_AN++) {
  //      printf("%d %d %d %d\n",GA_AN, iR, mapN2n[GA_AN-1]+1, mapN2rn[iR][GA_AN-1]);fflush(stdout);
  //    }
  //  }
  //}
...
//void set_FNAN_in_cell(int* FNAN_C, int** natn_C) {
//  /* set natn_C and FNAN_C */
//
//  int Gc_AN,h_AN,Gh_AN,Rn,itmp;
//
//  FNAN_C=(int*)malloc(sizeof(int)*(atomnum+1));
//  FNAN_C[0] = 0;
//
//  /* set FNAN_C */
//  for (Gc_AN=1; Gc_AN<=atomnum; Gc_AN++){
...
//  }
//
//  natn_C=(int**)malloc(sizeof(int*)*atomnum);
//  for (i=0; i<=atomnum; i++) natn_C[i]=(int*)malloc(sizeof(int)*(FNAN_C[i]+1));
//
//  /* set natn_C */
//  for (Gc_AN=1; Gc_AN<=atomnum; Gc_AN++){
//    itmp = 0
...
//void free_FNAN_in_cell(int* FNAN_C, int** natn_C) {
//
//  for (i=0; i<=atomnum; i++) free(natn_C[i]); free(natn_C);
//  free(FNAN_C);
//
//}
...
void buildMapRlist() {
  ...
  //  for (j=0; j<atomnum; j++) { 
  //    Elem[i][j]=(double***)malloc(sizeof(double**)*(FNAN[j+1]+1));
  //    for (k=0; k<=FNAN[j+1]; k++) {
  //      Elem[i][j][k]=(double**)malloc(sizeof(double*)*Norbperatom[j]);
  //      for (l=0; l<Norbperatom[j]; l++) {
  //        Elem[i][j][k][l]=(double*)malloc(sizeof(double)*Norbperatom[natn[j+1][k]-1]);
  //        for (m=0; m<Norbperatom[natn[j+1][k]-1]; m++) {
  ...
  //    wan2 = WhatSpecies[Gh_AN];
  //    TNO2 = Spe_Total_CNO[wan2];
  //    imap = MapR(atv_ijk[Rn][1],atv_ijk[Rn][2],atv_ijk[Rn][3]);
  //    for (i=0; i<TNO1; i++){
  //      for (j=0; j<TNO2; j++){
  //        /* fukuda Elem outside of the FNAN is not needed */
  //        Elem[imap][Gc_AN-1][h_AN][i][j]=Tmp_Vec[num];
  //        num++;
  //      }
  //    }
  //  }
  ...
  //MPI_Barrier(mpi_comm_level1);
  //MPI_Bcast( &Tmp_Vec[0], num, MPI_DOUBLE, Host_ID, mpi_comm_level1);
  //MPI_Barrier(mpi_comm_level1);
  //printf("End MPI_Bcast\n");fflush(stdout);
